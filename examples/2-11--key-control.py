...
class GLFrame(OpenGLFrame):
    ...
    def update_scene(self, dt):
        ...
        if self._input_api.is_key_pressed("left"):
            self._translation[0] -= distance
        if self._input_api.is_key_pressed("right"):
            self._translation[0] += distance
        if self._input_api.is_key_pressed("down"):
            self._translation[1] -= distance
        if self._input_api.is_key_pressed("up"):
            self._translation[1] += distance
